...
const initializeTreeSetInArray = (n) => { let g = []; for (let i = 0; i < n; i++) { g.push(new SplayTree()); } return g; };

// Accepted
const clearStars = (s) => {
    let f = initializeTreeSetInArray(26);
    let n = s.length, removeIds = new Set(), res = '';
    // pr(f.map(tree => tree.show()))
    for (let i = 0; i < n; i++) {
        if (s[i] == '*') {
           // pr(f.map(tree => tree.show()))
            for (let j = 0; j < 26; j++) {
                if (!f[j].isEmpty()) { // delete smallest char
                    let removeIdx = f[j].pollLast();
                    removeIds.add(removeIdx);
                    break;
                }
            }
        } else {
            f[ord(s[i]) - 97].insert(i);
            // pr("22", ord(s[i]) - 97, f.map(tree => tree.show()))
        }
    }
    // pr(removeIds)
    for (let i = 0; i < n; i++) {
        if (!removeIds.has(i) && s[i] != '*') res += s[i];
    }
    return res;
};
